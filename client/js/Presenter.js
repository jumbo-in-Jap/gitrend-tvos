...
var Presenter = {
    ...
    selectLanguage: function(event){
        _selectedElement = event.target
        var self = this
        var selectedLang = _selectedElement.getAttribute("attr")
        console.log("Selected: "+selectedLang)

        templateURL = _selectedElement.getAttribute("template"),
        presentation = _selectedElement.getAttribute("presentation");
        self.showLoadingIndicator(presentation)

        Api.getTrend(selectedLang,
          // Success
          function(response){
            _trendResult = response
            self.showLanguageTrandOnProductOnListWithBanner(response, selectedLang)
    ...
    showLanguageTrandOnProductOnListWithBanner: function(response,query){
      ...
      var index = resourceLoader.loadResource(`${resourceLoader.BASEURL}templates/ListWithBanner.xml.js`,
          function(resource) {
              var doc = Presenter.makeDocument(resource);
              _showProductDom = doc

              // change title
              var title = _showProductDom.ownerDocument.getElementById("page_title")
              title.innerHTML = query

              // make response dom
              var section = _showProductDom.getElementsByTagName("section").item(0)
              //_listTemplateElement = section.getElementsByTagName("listItemLockup").item(0).cloneNode(true)
              console.log("Response count:"+eval("response."+query+".length"))

              for(i = 0; i < eval("response."+query+".length");i++){
                trend = eval("response."+query+"[i]")
                var listTemplateElement = section.getElementsByTagName("listItemLockup").item(0).cloneNode(true)
                listTemplateElement.getElementsByTagName("img").item(0).setAttribute("src", trend.dev_img_url.replace(/\=40/g,"=460"))
                listTemplateElement.getElementsByTagName("title").item(0).innerHTML = trend.repo
                listTemplateElement.getElementsByTagName("title").item(1).innerHTML = trend.repo
                if (trend.star != ""){
                  listTemplateElement.getElementsByTagName("decorationLabel").item(0).innerHTML = "Star:"+trend.star
                }else{
                  listTemplateElement.getElementsByTagName("decorationLabel").item(0).innerHTML = " "
                }
                listTemplateElement.getElementsByTagName("subtitle").item(0).innerHTML = trend.org
                listTemplateElement.getElementsByTagName("description").item(0).innerHTML = trend.description.replace(/&/g, '&amp;')
               .replace(/</g, '&lt;')
               .replace(/>/g, '&gt;')
               .replace(/"/g, '&quot;')
               .replace(/'/g, '&apos;');
                section.appendChild(listTemplateElement)
              }
              section.removeChild(section.getElementsByTagName("listItemLockup").item(0))
    ...
    showLanguageTrandOnProduct: function(response,query){
      //resourceLoader = new ResourceLoader(this.BASEURL);
      //var index = resourceLoader.loadResource(`${options.BASEURL}templates/Index.xml.js`,
      var index = resourceLoader.loadResource(`${resourceLoader.BASEURL}templates/Product.xml.js`,
          function(resource) {
              var doc = Presenter.makeDocument(resource);
              _showProductDom = doc

              // change title
              var title = _showProductDom.ownerDocument.getElementsByTagName("title").item(0).innerHTML
              title.innnerHTML = query

              // make response dom
              var section = _showProductDom.getElementsByTagName("section").item(0)
              _productTemplateElement = section.firstChild.cloneNode(true)
              console.log("Response count:"+eval("response."+query+".length"))
              for(i = 0; i < eval("response."+query+".length");i++){
                trend = eval("response."+query+"[i]")
                var productTemplateElement = section.firstChild.cloneNode(true)
                //productTemplateElement.getElementsByTagName("badge").item(0).setAttribute("src", trend.dev_img_url)
                productTemplateElement.getElementsByTagName("title").item(0).innerHTML = trend.repo
                productTemplateElement.getElementsByTagName("subtitle").item(0).innerHTML = trend.org
                productTemplateElement.getElementsByTagName("description").item(0).innerHTML = trend.description
                section.appendChild(productTemplateElement)
              }
